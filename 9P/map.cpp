...
bool Map<KEY, T>::insert(Elem *& node, const KEY &key, const T &data, Elem *lastLeft) {
  ...
  if (node == 0) {
	...
	if (key < node->key) {
		...
		cout  << "\tgoing right\n";
  	if (node->rightThread) {
      //cout << "\tfixing for right threading\n";
      node->rightThread = false;
      //cout  << "\tinserting right\n";
			if (node->height == 0) {
				node->height = 1;
			}
			// if (node->left == nullptr) { // One right child
			// 	node->height = 1;
			// } else if (node->left != nullptr) { // One left child
...
// template <typename KEY, typename T>
// int Map<KEY, T>::height(Elem * _cur) {
// 	if (_root ==_root->left) {
// 		return 0;
// 	}
//
...
// 	}
//
// 	if (_cur ->left == nullptr && _cur->rightThread) { // No Children
//
// 		_cur->height = 0;
// 	}	else if (_cur->left == nullptr && !_cur->rightThread) { // One right child
// 		_cur->height = height(_cur->right) + 1;
// 	} else if (_cur->left != nullptr && _cur->rightThread) { // One left child
// 		_cur->height = height(_cur->left) + 1;
// 	} else if (_cur->left != nullptr && !_cur->rightThread) { // 2 children
// 		_cur->height = max(height(_cur->left), height(_cur->right)) + 1;
// 	}
//
// 	return _cur->height;
// }
